...
clc;
tic;
fileName = 'Cylinder_Stat_x15 Table(Dry)';
...
for j = 2:nSheets
    ...
    for k = 1:N
        ...
        if k==1
            ...
            title(['Dry', num2str(j1),': VWC = ',num2str(S(j1).VWC(k)),' Sat = ',num2str(S(j1).Saturation(k))]);
            set(gca,'fontsize',14,'fontweight','bold','Xlim',[0 2]);
            figName = ['Dry', num2str(j1),' VWC = ',num2str(S(j1).VWC(k)),' Sat = ',num2str(S(j1).Saturation(k))];
    ...
    title(['Dry', num2str(j1)]);
    set(gca,'fontsize',14,'fontweight','bold');
    figName = ['Dry ', num2str(j1)];
    saveas(gcf,[figName,'.fig']);
    saveas(gcf,[figName,'.jpg']);
end
save([fileName,'.mat'],'S');
