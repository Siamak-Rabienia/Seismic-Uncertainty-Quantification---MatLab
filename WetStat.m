...
load('Cylinder_Stat_x15 Table(Wet)');
Tmax =[S(1).ttMax(:);S(2).ttMax(:);S(3).ttMax(:);S(4).ttMax(:);S(5).ttMax(:);S(6).ttMax(:);S(7).ttMax(:);S(8).ttMax(:)];
Vmax =[S(1).VsMax(:);S(2).VsMax(:);S(3).VsMax(:);S(4).VsMax(:);S(5).VsMax(:);S(6).VsMax(:);S(7).VsMax(:);S(8).VsMax(:)];
